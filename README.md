...
Names on the console are written for a product reader first; the technical term is in the subtitle, the hover text, or the **Glossary** button in the top bar.

| Page | What it shows | How to read it |
| --- | --- | --- |
| **Overview** | Headline tiles, the signal timeline, the with-vs-without-alert chart, where the value comes from, and two queues | The first tile is estimated value (see Business impact). The timeline is every report and alert by arrival time: squares are alerts, circles are emails, color is the decision. "Needs a human" lists recent hand-offs with the reason the system would not act on its own; "Handled automatically" lists recent cases it handled itself. Every row opens the case in Signal replay. |
| **Business impact** | Dollar amounts per report, alert, and attack, and the cost model behind them | Five categories: engineer triage time saved, on-call interruptions avoided, revenue protected by keeping customers informed, wrong-team delays avoided by the ownership lookup, and security exposure avoided. The **Cost model** card is the whole model; edit any input and every figure recomputes. These are assumptions, not measurements: replace the defaults with your own before quoting them. |
| **Signal replay** | The board: one row per report or alert in arrival order, with grouping, service, priority, decision, actions, and value | Press **Play** to watch signals arrive. Click a row for the steps taken (the state trace), the actions planned with idempotency keys, the actual answer vs. the system's answer, what it was grouped with, and the original text exactly as the system saw it. The service chip says where the answer came from: **from alert** (near-certain, `grounded:telemetry`), **guessed from text** (`inferred:model`), or **not sure** (it abstained). |
| **Accuracy** | Every metric from Section 5, sliced | The first chart is the same customer emails with and without the matching alert (grounding lift). "Is its confidence trustworthy?" plots stated confidence against how often it was right; on the diagonal is perfect (calibration). "Paths through the pipeline" shows where hand-offs cluster. "All metrics" is the brief's results table, never averaged across sources. |
| **Security** | Attacks stopped by type, and each attack with what it tried | "Noticed" is whether the detector flagged it; "Stopped" is whether any of the five checks failed. The row to look for is one that was stopped without being noticed: that is the architecture holding when the detector did not. |
| **Misses** | Every case where the service was wrong, with the steps it took | The Accuracy page says how often; this page says why. Each entry opens in Signal replay. |
| **Integrations** | Connect cards for Slack, Email, GitHub, Sentry, Datadog, and Linear | Status reflects what `doctor` found at build time. Each card's Connect dialog has a Direct / MCP transport switch and shows the `.env` lines to paste; the page never makes calls itself. Sources feed the pipeline; only the executor holds a credential that can send anything. |
...
- **Decisions** (tiers) are the gate's outcomes: **Handled** (auto), **Needs approval** (propose), **Needs a human** (escalate). No model-inferred localization can reach Handled because the `ungrounded_localization` rule caps confidence at 0.70; every email it handled itself had its service from an alert.
- **Estimated value** attributes dollars by what the pipeline did with each signal, not by what a human would have done. A grouped signal earns an interruption avoided; a reporter kept informed earns revenue protected; a case the text guess would have sent to the wrong team earns a wrong-team delay avoided; a stopped attack earns its type's exposure. Sums are on the Business impact page with the assumptions beside them.
